...
public class GrpcStreamingService : IGrpcStreamingService
{
            ...
            ctx.ReceiveTask = Task.Run(async () =>
            {
                try
                {
                    while (moveNextMethod != null && currentProperty != null)
                    {
                        ...
                        ctx.ReceivedMessages.Add(_jsonConverter.MessageToJson((IMessage)current!));
                    }
                }
                catch (Exception ex)
                {
    ...
    private static CallOptions BuildCallOptions(GrpcRequestPayload payload)
    {
        ...
        return new CallOptions(metadata, DateTime.UtcNow.AddSeconds(payload.TimeoutSeconds));
    }
